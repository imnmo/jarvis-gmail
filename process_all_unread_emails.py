...
def parse_email_data(gmail: Resource, message_info: Dict[str, Union[str, List[str]]]) -> Dict[str, Union[str, List[str]]]:
    ...
    try:
        msg = gmail.users().messages().get(
            userId='me',
            id=message_info['id'],
            format='full' #there are other types like 'raw', 'minimal', 'metadata'
        ).execute()
    except Exception as e:
...
def evaluate_email(email_data: Dict[str, Union[str, List[str]]], user_first_name: str, user_last_name: str, client: OpenAI) -> bool:
    ...
    system_message: Dict[str, str] = {
        ...
        "content": (
            ...
            "Also, remember I get emails in German, so you need to evaluate that as well"
            ...
            "such as needing to send a payment via Paypal or Klarna or Mahnung, but ignore requests for non-essential actions "
    ...
    try:
        ...
        print(f"Failed to evaluate email with gpt-3.5-turbo: {e}")
        return False
